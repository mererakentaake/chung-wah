...
function SelectField({ label, value, onChange, options, placeholder, T, width, error, refCb, disabled }: any) {
  const isEmpty = !value;
  return (
    <div ref={refCb} style={{ display: "flex", flexDirection: "column", gap: 4, width: width || FIELD_W, minWidth: FIELD_MIN }}>
      {label && <label style={{ fontSize: 11, color: disabled ? T.placeholder : T.muted, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" as const }}>{label}</label>}
      <select value={value} onChange={(e: any) => onChange(e.target.value)}
        disabled={disabled}
        className={isEmpty ? "cia-placeholder" : ""}
        style={{
          ...inputStyle(T),
          color: disabled ? T.placeholder : (isEmpty ? T.placeholder : T.text),
        ...
        onFocus={(e: any) => { if (!disabled) e.target.style.borderColor = error ? T.danger : T.accent; }}
        onBlur={(e: any) => e.target.style.borderColor = error ? T.danger : T.border}
      >
...
// ─── Form State ───────────────────────────────────────────────────────────────
const CLUSTERS_BY_REGION: Record<string, string[]> = {
  "Region 1": [
    "West Are'are Cluster",
    "East Are'are Cluster",
    "Central Malaita Cluster",
    "North Malaita Cluster",
    "Temotu Cluster",
    "Isabel Cluster",
    "Makira Cluster",
    "Small Malaita Cluster",
  ],
  "Region 2": [
    "Honiara Urban Cluster",
    "Guadalcanal Cluster",
    "Renbel Cluster",
    "East Western Cluster",
    "Central Western Cluster",
    "Shortland Islands Cluster",
    "Choiseul Cluster",
  ],
};
...
function DataCollectionForm({ forms, setForms, setSavedForms, currentIndex, setCurrentIndex, setPage, T }: any) {
  ...
  const handleSave = () => {
    ...
    // All valid — persist to localforage, reset form, return to dashboard
    const completed = { ...form, savedAt: new Date().toISOString() };
    setSavedForms((prev: any[]) => {
      const updated = [...prev, completed];
      localforage.setItem("cia_saved_forms", updated);
      return updated;
    });
    setForms([emptyForm()]);
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 0 60px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <SelectField T={T} label="Region" value={form.region} onChange={(v: string) => { update("region", v); update("cluster", ""); }} options={REGIONS} placeholder="Select Region" width="40%" error={errors.region} refCb={setRef("region")} />
        <SelectField T={T} label="Cluster" value={form.cluster} onChange={(v: string) => update("cluster", v)} options={form.region ? (CLUSTERS_BY_REGION[form.region] ?? []) : []} placeholder={form.region ? "Select Cluster" : "Select Region first"} width="60%" error={errors.cluster} refCb={setRef("cluster")} disabled={!form.region} />
...
const SUMMARY_COLS = [
  // col 0 — scrolls away to the left
  { key: "region",               label: "Region",                                                            group: null,                        subgroup: null,    width: 70  },
  // col 1 — scrolls away to the left
  { key: "cluster",              label: "Cluster",                                                            group: null,                        subgroup: null,    width: 90  },
  // col 2 — sticky column (freezes once it reaches the left edge)
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          {options.length === 0 ? (
            <div style={{
              ...
              No {title.replace("Select a ", "").toLowerCase()}s have been added yet.
            </div>
          ) : options.map((opt: string) => (
...
function Summary({ forms, T }: any) {
  ...
  const modalOptions: string[] =
    ...
    viewLevel === "Cluster"  ? Object.values(CLUSTERS_BY_REGION).flat().sort() :
    // Locality is not yet a data field collected by the form — no localities
    // exist yet, so the modal always shows an empty list with a notice.
    viewLevel === "Locality" ? [] :
    [];

  // Filter forms based on selection. Locality has no real data field yet,
  // so selecting it always yields zero results.
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const XF_NOTE1   = 3;  // Calibri11B left+wrap (note row G)
      const XF_NOTE2   = 4;  // Times Ext Roman vcenter
      const XF_NOTE3   = 5;  // Calibri11B vcenter
      const XF_COMMENT = 12; // TNR12 + full thin border + left-align + wrap (Comments column)
      ...
      displayForms.forEach((f: any, idx: number) => {
        ...
        setS(34,r,d(f.spiritualHealth),XF_DATA);     setS(35,r,d(f.comments),XF_COMMENT);
      });

      // With 0 records, dataRowStart itself is just a blank placeholder row,
      // so only +1 more row is needed for a single blank gap before notes.
      // With 1+ records, lastDataRow is real data, so the same +1 lands
      // correctly one row after the actual last record — also a single gap.
      const lastDataRow = displayForms.length > 0
        ? dataRowStart + displayForms.length - 1
        : dataRowStart - 1; // virtual "last row" one above the placeholder
      const notesStart   = lastDataRow + 2; // exactly one blank row gap before notes
      // Borders only apply to rows that actually hold real records — the
      // placeholder gap row(s) before the notes stay completely unstyled.
      const lastRealDataRow = dataRowStart + displayForms.length - 1;
      const hasRealData = displayForms.length > 0;
      ...
      const ROW_HT: Record<number, number> = { 2:18.75, 3:31.5, 4:31.5 };
      if (hasRealData) ROW_HT[dataRowStart] = 41.25; // first data row matches comment wrap height
      ROW_HT[notesStart] = 20.25; // only the first row of the merged G-note gets an explicit height
      ...
      // Column AI (Comments) auto-expands based on the longest comment in this
      // export. bestFit="1" alone is not reliably honoured by every spreadsheet
      // app (WPS in particular), so the base width is computed directly here.
      const longestComment = displayForms.reduce((max: number, f: any) => {
        const len = (f.comments ?? "").length;
        return len > max ? len : max;
      }, 0);
      // ~1.1 width units per character is a reasonable approximation for
      // Calibri/Times New Roman at this font size; floor at the default 18.14.
      const aiWidth = Math.max(18.14, Math.min(60, longestComment * 1.1));

      const COL_WIDTHS = [
        11.43359375, 20.84765625, 15.73828125, 10.76171875, 13.31640625, 13.046875, 14.390625, 16.140625,
        8.0703125, 13.046875, 13.0, 13.0, 13.0, 13.0, 13.0, 13.0, 13.0, 13.0, 13.0, 13.0, 13.0, 13.0, 13.0,
        16.140625, 13.046875, 15.19921875, 13.046875, 13.0, 13.71875, 15.87109375, 16.54296875, 13.046875, 13.0, 13.0, aiWidth,
      ];
      ...
      const sheetXml =
        ...
        `<sheetView workbookViewId="0">` +
          ...
          `<selection pane="topRight" activeCell="C1"/>` +
        `</sheetView>` +
        ...
        (hasRealData
          ? `<conditionalFormatting sqref="A${dataRowStart}:AI${lastRealDataRow}">` +
  ...
  // Region + Cluster scroll normally to the left of the sticky CIA column.
  // CIA (SUMMARY_COLS[2]) is the sticky column — it freezes at the left edge
  // once Region/Cluster have scrolled past, and everything after it scrolls underneath.
  const leadCols   = SUMMARY_COLS.slice(0, 2);  // Region, Cluster
  const stickyCol   = SUMMARY_COLS[2];           // CIA
  const scrollCols  = SUMMARY_COLS.slice(3);     // everything after CIA
          ...
          <div style={{ overflowX: "auto", borderRadius: 10, border: `1.5px solid ${T.border}`, boxShadow: `0 2px 12px ${T.border}66` }}>
            <table style={{ borderCollapse: "collapse", tableLayout: "auto" as const, width: "100%" }}>
              <thead>
                <tr>
                  {leadCols.map((col) => (
                    <th key={col.key} rowSpan={3} style={{
                      ...thBase, background: T.surface, minWidth: col.width,
                  ...
                  <th rowSpan={3} style={{
                    ...
                    background: T.surface, minWidth: stickyCol.width,
                    ...
                    <tr key={f.id} style={{ background: rowIdx % 2 === 0 ? T.card : T.surface }}>
                      {leadCols.map((col) => (
                        <td key={col.key} style={{
                          ...tdBase, background: "inherit", minWidth: col.width,
...
// ─── Cycle Report ─────────────────────────────────────────────────────────────
function CycleReport({ forms, T }: any) {
  ...
  // Unique CIA names from all saved forms
  const allCIAs = Array.from(new Set(forms.map((f: any) => f.cia).filter(Boolean))) as string[];
  const filteredCIAs = allCIAs.filter((c) => c.toLowerCase().includes(searchText.toLowerCase()));

  // All records for the selected CIA, sorted by date ascending
  const ciaForms = selectedCIA
    ? [...forms.filter((f: any) => f.cia === selectedCIA)].sort(
        (a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime()
      )
    : [];

  // Build "Date from/to" for each record:
  // Each row's range = that record's date → next record's date (or today if last)
  ...
  const cycleRows = ciaForms.map((f: any, i: number) => {
    const nextDate = i < ciaForms.length - 1 ? ciaForms[i + 1].date : today;
    return { ...f, dateFrom: f.date, dateTo: nextDate };
  });

  // Reuse SUMMARY_COLS but replace the first col (cia) with dateFromTo
  const cycleCols = [
    { key: "dateFromTo", label: "Date from/to", group: null, sticky: true },
    ...SUMMARY_COLS.slice(1),
  ];

  const scrollCols = cycleCols.slice(1);
  const groupColor: Record<string, string> = {
    "Core Activities":            T.accent + "33",
    "Human Resource Development": T.accentLight + "44",
  };
  const hrdCellBg   = T.accentLight + "18";
  const thBase: React.CSSProperties = {
    padding: "4px 6px", textAlign: "center", fontSize: 9, fontWeight: 700,
    color: T.text, whiteSpace: "pre-line" as const, verticalAlign: "middle",
    border: `1px solid ${T.border}`, lineHeight: 1.25,
  };
  ...
  return (
    <div style={{ padding: "0 0 40px 0" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px 6px" }}>
        <div ref={searchRef} style={{ position: "relative", flex: 1, maxWidth: 280 }}>
          <input
            value={searchText}
            onChange={(e: any) => { setSearchText(e.target.value); setSearchOpen(true); }}
            onFocus={() => setSearchOpen(true)}
            placeholder="Search CIA..."
            style={{
              width: "100%", padding: "8px 12px", borderRadius: 8,
              border: `1.5px solid ${T.accent}`, background: T.card,
              color: T.text, fontSize: 13, boxSizing: "border-box" as const,
            }}
          />
          {searchOpen && filteredCIAs.length > 0 && (
            <div style={{
              position: "absolute", top: "100%", left: 0, right: 0, zIndex: 999,
              background: T.card, border: `1px solid ${T.border}`, borderRadius: 8,
              maxHeight: 200, overflowY: "auto" as const, boxShadow: "0 4px 16px #0003",
            }}>
              {filteredCIAs.map((cia) => (
                <div key={cia} onClick={() => { setSelectedCIA(cia); setSearchText(cia); setSearchOpen(false); }}
        ...
        <button onClick={() => setExportMsg("PDF export coming soon.")}
          disabled={!selectedCIA || exporting}
          style={{
            marginLeft: 10, padding: "8px 14px", borderRadius: 8, border: "none",
            background: selectedCIA ? T.accent : T.border,
            color: selectedCIA ? "#fff" : T.muted,
            cursor: selectedCIA ? "pointer" : "not-allowed",
            fontSize: 12, fontWeight: 700, whiteSpace: "nowrap" as const,
          }}>
          ↓ Export PDF
        </button>
      </div>
      {exportMsg && <div style={{ color: T.success, fontSize: 12, padding: "0 14px 6px" }}>{exportMsg}</div>}

      {!selectedCIA ? (
        <div style={{ textAlign: "center", color: T.muted, marginTop: 40, fontSize: 14 }}>
          Search and select a CIA to view its cycle report
        </div>
      ) : (
        <>
          <div style={{ textAlign: "center", fontFamily: "Georgia, serif",
            fontSize: 15, fontWeight: 700, color: T.heading, padding: "6px 12px 8px",
            letterSpacing: "0.04em", textTransform: "uppercase" as const }}>
            Cycle of Growth for {selectedCIA} (CIA)
          </div>

          {cycleRows.length === 0 ? (
            <div style={{ textAlign: "center", color: T.muted, marginTop: 20, fontSize: 13 }}>
              No records found for this CIA
            </div>
          ) : (
            <div style={{ overflowX: "auto" as const, WebkitOverflowScrolling: "touch" as any }}>
              <table style={{ borderCollapse: "collapse", fontSize: 11, minWidth: 900 }}>
                <thead>
                  {/* ── Row A: top header row ── */}
                  <tr>
                    {/* Date from/to: sticky, spans 2 rows */}
                    <th rowSpan={2} style={{
                      ...thBase, minWidth: 100, position: "sticky" as const, left: 0, zIndex: 4,
                      background: T.tableHeader,
                    }}>Date from/to</th>

                    {/* Cluster: spans 2 rows (no group above it) */}
                    <th rowSpan={2} style={{ ...thBase, minWidth: 80, background: T.tableHeader }}>
                      {SUMMARY_COLS.find((c:any)=>c.key==="cluster")?.label}
                    </th>

                    {/* Rural or Urban → No. of households connected: each spans 2 rows */}
                    {["ruralUrban","generalPopulation","totalHouseholds","individualsConnected","householdsConnected"].map((k) => (
                      <th key={k} rowSpan={2} style={{
                        ...thBase,
                        minWidth: SUMMARY_COLS.find((c:any)=>c.key===k)?.width ?? 80,
                        background: T.tableHeader,
                      }}>{SUMMARY_COLS.find((c:any)=>c.key===k)?.label}</th>
                    ))}

                    {/* Core Activities group header */}
                    <th colSpan={15} style={{ ...thBase, background: groupColor["Core Activities"] }}>
                      Core Activities
                    </th>

                    {/* HRD cols: each spans 2 rows */}
                    {["book1","totalRuhi","newHumanResources","totalHumanResources","accompany"].map((k) => (
                      <th key={k} rowSpan={2} style={{
                        ...thBase,
                        minWidth: SUMMARY_COLS.find((c:any)=>c.key===k)?.width ?? 90,
                        background: T.accentLight + "44",
                      }}>{SUMMARY_COLS.find((c:any)=>c.key===k)?.label}</th>
                    ))}

                    {/* No. of pockets → Comments: each spans 2 rows */}
                    {["pockets","regularUndertakings","localAssembly","socialAction","localLeaders","spiritualHealth","comments"].map((k) => (
                      <th key={k} rowSpan={2} style={{
                        ...thBase,
                        minWidth: SUMMARY_COLS.find((c:any)=>c.key===k)?.width ?? 80,
                        background: T.tableHeader,
                  ...
                  {/* ── Row B: subgroup row (only Core Activities subgroups) ── */}
                  <tr>
                    {[
                      { label: "Children's Classes",   count: 3 },
                      { label: "Junior Youth Groups",  count: 3 },
                      { label: "Study Circles",        count: 3 },
                      { label: "Devotional Meetings",  count: 3 },
                      { label: "Total Activities",     count: 3 },
                    ].map((sub) => (
                      <th key={sub.label} colSpan={sub.count} style={{
                        ...thBase, background: groupColor["Core Activities"] + "88",
                      }}>{sub.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {cycleRows.map((row: any, idx: number) => {
                    const bg = idx % 2 === 0 ? T.card : T.tableAlt;
                    const dateLabel = `${formatDate(row.dateFrom)} – ${formatDate(row.dateTo)}`;
                    const n = (v: any) => (v !== "" && v !== undefined && v !== null ? v : "");
                    const tot = (keys: string[]) => {
                      const sum = keys.reduce((s: number, k: string) => {
                        const [act, sub] = k.split("_");
                        return s + (Number(row.activities?.[act]?.[sub]) || 0);
                      }, 0);
                      return sum > 0 ? String(sum) : "";
                    };
                    return (
                      <tr key={row.id ?? idx}>
                        <td style={{ ...thBase, fontWeight: 600, position: "sticky" as const,
                          left: 0, zIndex: 2, background: bg, fontSize: 10,
                          boxShadow: "2px 0 4px #0002" }}>
                          {dateLabel}
                        </td>
                        {/* cluster */}
                        <td style={{ ...thBase, fontWeight: 400, background: bg, minWidth: 80 }}>{n(row.cluster)}</td>
                        {/* ruralUrban → householdsConnected */}
                        {["ruralUrban","generalPopulation","totalHouseholds","individualsConnected","householdsConnected"].map((k) => (
                          <td key={k} style={{ ...thBase, fontWeight: 400, background: bg, minWidth: 60 }}>{n(row[k])}</td>
                        ))}
                        {/* Core Activities: cc, jy, sc, dm, tot */}
                        {[
                          ["children","cc"],["juniorYouth","jy"],["studyCircle","sc"],["devotional","dm"]
                        ].map(([act]) => (["no","att","fof"].map((sub) => (
                          <td key={`${act}_${sub}`} style={{ ...thBase, fontWeight: 400, background: bg, minWidth: 50 }}>
                            {n(row.activities?.[act]?.[sub])}
                          </td>
                        ))))}
                        {/* Total Activities */}
                        <td style={{ ...thBase, fontWeight: 400, background: bg, minWidth: 50 }}>{tot(["children_no","juniorYouth_no","studyCircle_no","devotional_no"])}</td>
                        <td style={{ ...thBase, fontWeight: 400, background: bg, minWidth: 50 }}>{tot(["children_att","juniorYouth_att","studyCircle_att","devotional_att"])}</td>
                        <td style={{ ...thBase, fontWeight: 400, background: bg, minWidth: 50 }}>{tot(["children_fof","juniorYouth_fof","studyCircle_fof","devotional_fof"])}</td>
                        {/* HRD */}
                        {["book1","totalRuhi","newHumanResources","totalHumanResources","accompany"].map((k) => (
                          <td key={k} style={{ ...thBase, fontWeight: 400, background: bg, minWidth: 80 }}>{n(row[k])}</td>
                        ))}
                        {/* pockets → comments */}
                        {["pockets","regularUndertakings","localAssembly","socialAction","localLeaders","spiritualHealth","comments"].map((k) => (
                          <td key={k} style={{ ...thBase, fontWeight: 400, background: bg, minWidth: 70 }}>{n(row[k])}</td>
                        ))}
          ...
          <div style={{ color: T.muted, fontSize: 11, padding: "6px 14px" }}>
            {cycleRows.length} cycle{cycleRows.length !== 1 ? "s" : ""} · {selectedCIA}
          </div>
        </>
      )}
    </div>
...
export default function App() {
  ...
  // savedForms = all submitted records, persisted to localforage
  const [savedForms, setSavedForms] = useState<any[]>([]);
  useEffect(() => {
    localforage.getItem<any[]>("cia_saved_forms").then((stored) => {
      if (stored && Array.isArray(stored)) setSavedForms(stored);
    });
  }, []);
    ...
    <div style={{ position: "fixed", inset: 0, display: "flex", flexDirection: "column", background: T.bg, overflow: "hidden" }}>
      ...
      <main style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "0 0 24px", animation: "slideUp 0.2s ease", background: T.bg }}>
        ...
        <div style={{ padding: "0 16px" }}>
          ...
          {page === "form"       && <DataCollectionForm T={T} forms={forms} setForms={setForms} setSavedForms={setSavedForms} currentIndex={currentFormIndex} setCurrentIndex={setCurrentFormIndex} setPage={navigateTo} />}
          {page === "summary"    && <Summary T={T} forms={savedForms} />}
          {page === "cycle"      && <CycleReport T={T} forms={savedForms} />}
