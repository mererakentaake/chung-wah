...
export default function Profile() {
  ...
  // Build the profile title heading
  const profileTitle = isParent
    ? `${profile?.title ? profile.title + '. ' : ''}${profile?.displayName || ''}${profile?.displayName ? "'s Profile" : 'Your Profile'}`
  ...
  return (
    <div className="min-h-screen mesh-bg flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-28">
        ...
        {/* Parent heading — shown instead of avatar */}
        {isParent && (
          <div className="mb-6 px-1">
            <h2 className="font-display font-bold text-white text-xl leading-snug">{profileTitle}</h2>
            <p className="text-white/40 text-sm font-body mt-0.5">{user?.email}</p>
          </div>
        )}
